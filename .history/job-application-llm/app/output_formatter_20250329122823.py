...
class OutputFormatter:
    ...
    def create_docx(self, content, job_data, candidate_data, content_type):
        ...
        try:
            ...
            # Generate filename with truncated company name
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            # Limit company name to 30 characters and clean it
            company_name = job_data['company_name'][:30].replace(' ', '_')
            company_name = ''.join(c for c in company_name if c.isalnum() or c == '_')  # Remove special characters
