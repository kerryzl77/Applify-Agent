...
class OutputFormatter:
    ...
    def _format_cover_letter_docx(self, doc, content, job_data, candidate_data):
        ...
        try:
            ...
            # Add company info (only if meaningful values are available)
            if job_data.get('company_name') and job_data.get('company_name') not in ["Unknown Company", "a Scientist"]:
                doc.add_paragraph()  # Add space after date
                company_info = doc.add_paragraph()
                company_info.alignment = WD_ALIGN_PARAGRAPH.LEFT
                company_info.add_run(job_data['company_name'])
                if job_data.get('location') and job_data['location'] not in ["Unknown Location", "None"]:
            ...
            # Remove any "Unknown Company" or "Unknown Location" references from the content
            paragraphs = content.split('\n')
            for paragraph in paragraphs:
                if paragraph.strip():
                    p = doc.add_paragraph()
                    p.alignment = WD_ALIGN_PARAGRAPH.LEFT
                    # Clean up any potential "Unknown" references
                    cleaned_paragraph = paragraph.strip()
                    cleaned_paragraph = cleaned_paragraph.replace("at Unknown Company", "")
                    cleaned_paragraph = cleaned_paragraph.replace("at a Scientist", "")
                    cleaned_paragraph = cleaned_paragraph.replace("Unknown Location", "")
                    p.add_run(cleaned_paragraph)
            
            # Add signature
            doc.add_paragraph()  # Add space before signature
            signature = doc.add_paragraph()
            signature.alignment = WD_ALIGN_PARAGRAPH.LEFT
            signature.add_run("Best regards,\n" + candidate_data['personal_info']['name'])
