...
class OutputFormatter:
    ...
    def create_docx(self, content, job_data, candidate_data, content_type):
        ...
        try:
            ...
            company_name = job_data.get('company_name', 'Unknown')[:30]  # Limit length
            company_name = ''.join(c for c in company_name if c.isalnum() or c == '_')  # Sanitize
            ...
            # Save the document
            doc.save(filepath)
            print(f"Successfully created DOCX file: {filepath}")
    ...
    def _format_cover_letter_docx(self, doc, content, job_data, candidate_data):
        ...
        try:
            ...
            # Add company info (only if available)
            if job_data.get('company_name') and job_data.get('company_name') != "Unknown Company":
                company_info = doc.add_paragraph()
                company_info.alignment = WD_ALIGN_PARAGRAPH.LEFT
                company_info.add_run(job_data['company_name'])
                if job_data.get('location') and job_data['location'] != "Unknown Location":
