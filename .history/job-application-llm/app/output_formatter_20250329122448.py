...
class OutputFormatter:
    ...
    def convert_to_pdf(self, docx_path):
        """Convert DOCX to PDF and ensure it's only one page."""
        try:
            # Generate PDF path
            pdf_path = docx_path.replace('.docx', '.pdf')
            
            if self.in_docker:
                # For Docker environment, use LibreOffice
                try:
            ...
            if os.path.exists(pdf_path):
                # Truncate to one page if needed
                ...
                if len(reader.pages) > 1:
                    ...
                    print(f"PDF truncated to first page: {pdf_path}")
